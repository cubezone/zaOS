...
//	addListener(m_list,ZaEvent::EVENT_LIST_SELECT,&ZaScreenDict::onclick);
	...
	m_back->setSize(m_width - 100, m_height-100, ICONSIZE, ICONSIZE);
	AddChild(m_back);
...
//	AddChild(m_scroll);
	...
	m_lab->setSize(20,  v_h+70+10+ (m_height - v_h -70) *0.3f, m_width - 100, (m_height - v_h -70) *0.6f);
	m_lab->setcontent("explian:","UTF-8");
	AddChild(m_lab);

	m_list = new ZaList();
	m_list->m_rowheight = 50;
	m_list->setSize(20, v_h+70, m_width - 100, (m_height - v_h -70) *0.3f);
	...
	m_copy->setSize(m_width - 100,280+60, ICONSIZE, ICONSIZE);
	AddChild(m_copy);
	addListener(m_copy, ZaEvent::EVENT_CLICK, &ZaScreenDict::onclick);

	l_img = new ZAImage();
	l_img->LoadImg("app/native/res/paste.png", ICONSIZE, ICONSIZE);
	m_paste = new ZaButton();
	m_paste->setImage(l_img);
	m_paste->setSize(m_width - 100,280, ICONSIZE, ICONSIZE);
	AddChild(m_paste);
...
void ZaScreenDict::onclick(ZaObj * obj, ZaEvent *env)
{
	...
	{
		char * p=NULL;
		if (get_clipboard_data("text/plain",&p)>0 && p)
		{
			if(strlen(p)>50)
			{
	...
	if (obj == m_back)
	{
		...
		if (m_srceencaller  != NULL)
			l_src = m_srceencaller;
		string p = "ret";
