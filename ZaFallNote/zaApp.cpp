...
//
	...
	l_main = new zaScreenMain();
	l_main->setSize(0,0,m_width,m_height);
	l_main->init();
	ZaResLib::getInstance()->addScreen("ZaScreenMain",l_main);
	...
	m_active = (ZaScreenBase *)ZaResLib::getInstance()->getScreen("ZaScreenMain");
	// active open with  param;
	m_active->tweener(10,0);

//	bbcubezone::ZaSoundOGG og;
//	og.run("/accounts/1000/shared/music/aa.ogg");
