...
uses
  ...
  MPlayer,mmsystem, ActnList, MCBtn;
...
begin
    ...
    l_opendlg.Filter :=  'NFS files (*.nfs)|All files (*.*)';
...
begin
   Timer1.Enabled := false;
  mp3player.Close;
 OpenDlg.Filter :=  'Mp3 files (*.mp3)|Asf files (*.asf)|wmv files (*.wmv)|All files (*.*)';
...
begin
    l_opendlg :=  TOpendialog.Create(self);
    l_opendlg.Filter :=  'Jpg files (*.jpg)|Jpeg files (*.jpeg)|All files (*.*)';
    l_opendlg.Options:= OpenDlg.Options + [ofFileMustExist];
    if l_opendlg.Execute then
    begin
