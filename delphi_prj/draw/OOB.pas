...
begin
 x := round(x *100/m_zoom);
 y := round(y *100/m_zoom);
...
begin
    ...
    for i := 0 to m_list.Count-1  do
          Tobj(m_list.Items[i]).SavetoFile(i,filehandle);
end;
...
var
   ...
   o,o1 :tobj;
  imagefile :TmetaFile;
   l_image : TMetafileCanvas;
begin
{  imagefile := TMetafile.Create;
  imagefile.Width := 1024;
  imagefile.Height := 768;
   try
    l_image := TMetafileCanvas.Create(imagefile, image.Canvas.Handle);
}
  image.Canvas.Brush.Color := clwhite;
...
//   paintbox1.Width := round(max(maxx,scrollbox1.Width)* 100 /m_zoom);
//   paintbox1.Height := round(max(maxy,scrollbox1.Height)* 100 /m_zoom);

   //paintbox1.Canvas.CopyRect(image.ClientRect,image.Canvas,image.ClientRect);
   paintbox1.canvas.StretchDraw(
   rect(round(image.ClientRect.Left*m_zoom/100),
   round(image.ClientRect.Top*m_zoom/100),
   round(image.ClientRect.Right*m_zoom/100),
   round(image.ClientRect.Bottom*m_zoom/100)),
...
//           l_obj.m_selected := 0;
  ...
  mymetafile.Height := l_rect.Bottom - l_rect.Top + 30;
   try
    ...
    mYmETAFILE.SaveToFile('C:\\MM.emf');
...
begin
  l_p := paintbox1.ScreenToClient(Mouse.CursorPos);
   l_p.x := round(l_p.X *100/ m_zoom );
   l_p.y := round(l_p.y * 100/m_zoom );
   for B := (m_List.Count - 1) downto 0 do
   begin
       t := TObj(m_List.Items[B]);
...
begin
  l_p := paintbox1.ScreenToClient(Mouse.CursorPos);
   main.FormMouseDown(Sender,mbLeft,[],round(l_p.x * 100 /m_zoom),round(l_p.y * 100 /m_zoom));
end;
...
{     for b:=0 to m_list.Count -1 do
     begin
     if  TObj(m_List.Items[B]).m_zoom > 20 then
       TObj(m_List.Items[B]).m_zoom := TObj(m_List.Items[B]).m_zoom -20;
     end;
}    if m_zoom < 200 then
     begin
        m_zoom := m_zoom + 20;
...
{     for b:=0 to m_list.Count -1 do
     begin
     if  TObj(m_List.Items[B]).m_zoom < 200 then
       TObj(m_List.Items[B]).m_zoom := TObj(m_List.Items[B]).m_zoom + 20;
     end;
}    if m_zoom > 20 then
     begin
