...
type
TPmysql = class(TObject)
private
        m_Query: string;
	m_Result: PMYSQL_RES;
	m_Row: PMYSQL_ROW;
	m_Field: PMYSQL_FIELD;
        m_field_num : integer;
        m_MySql :PMYSQL;
public
 //       constructor Create(const Msg: string);

	procedure connect(const user, pass, db : string);

	procedure query(const sql : string);
	
	function fetch_row():Integer;

	function fetch_field(i :integer) :PMYSQL_FIELD;
...
procedure TpMysql.connect(const user, pass, db : string);
begin
    m_mysql := mysql_init(nil);
   if m_mysql = nil then Fail('Couldn''t init PMYSQL object');
   if mysql_real_connect(m_MySQL, pChar('localhost'), pChar(User), pChar(Pass), nil, 3306, nil, 0) = nil then Fail;
end;

procedure TpMysql.query(const sql : string);
begin
    m_query := sql;
    if mysql_query(m_mysql,pchar(sql)) <> 0 then Fail;
    m_Result := mysql_use_result(m_mysql);
    m_field_num := mysql_num_fields(m_Result);
    if m_Result = nil then Fail;
end;

function TpMysql.fetch_row() : integer;
begin
...
function TpMysql.fetch_field(i :integer) :PMYSQL_FIELD;
begin
   m_field := mysql_fetch_field(m_Result);
   result := m_field;
end;

procedure TpMysql.discount();
begin
    mysql_close(m_MySQL);
end;


procedure TpMysql.Fail(Msg: string = '');
begin
        // mysql_errno, mysql_error
	if Msg = '' then Msg := Format('%d - %s', [mysql_errno(m_MySQL), mysql_error(m_MySQL)]);
