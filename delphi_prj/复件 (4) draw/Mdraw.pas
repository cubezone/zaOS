...
begin
   ...
   if richedit1.Lines.Count = 1 then exit;
    ...
    l_my.query('select max(doc_id) from tab_doc');
     ...
     l_doc_id := format('%06d',[strtoint(l_doc_id)+1]);
...
//        "C:\Documents and Settings\sysdaemon.LINKONE\My Documents\te.doc"
   ...
   else if (copy(listbox2.Items.Strings[i],0,1) = '[') and (copy(listbox2.Items.Strings[i],5,1) = ']') then
...
var 
    ...
    sql : string;
begin    b:=0;
     if application.MessageBox('Are you sure to update the book to Remove ?','Information',MB_OKCANCEL) = ID_OK then
     begin

     sql := 'update  tab_doc set remark = ''Remove'' where doc_id = '''+ m_bookid+'''';
    l_my.query(sql);
...
{  l_str:= memo5.Text;
  ...
  l_y := 0;
  l_rectop := 0;
  image2.Width := scrollbox1.Width-25;
  ...
  begin
   ...
   begin
     l_len := l_len + image2.Canvas.TextWidth(l_str[i]);
     if (l_len > image2.Width ) or (l_str[i] = #10) then
       break;

   end;
    if l_y > image2.Height then
       image2.Height := l_y + 30;
    if ByteToCharIndex(l_str,i) = ByteToCharIndex(l_str,i+1)then
     i:= i + 1;
    image2.Canvas.TextOut(5,l_y+2,copy(l_str,0,i));
    l_y := l_y+ round(image2.Canvas.Textheight(copy (l_str,0,i))*1.5);
    l_rectbuttom := l_y;
    l_y := l_y + 4;

    if l_str[i] = #10 then
    begin
    image2.Canvas.MoveTo(0,l_rectop);
    image2.Canvas.LineTo(image2.Width-1,l_rectop);
    image2.Canvas.LineTo(image2.Width-1,l_rectbuttom);
    image2.Canvas.LineTo(0,l_rectbuttom);
    image2.Canvas.LineTo(0,l_rectop);
     l_rectop := l_y;
    end;
           ...
           paintbox4.Canvas.Pen.Color := clblack;
             ...
             begin
                ...
                begin
                 Randomize;
                 p:= Random(9);
                 tpic(m_pic_list.Items[p]).m_Pic.LoadFromFile(copy(l_str,8,200));
                  tpic(m_pic_list.Items[p]).m_filename := copy(l_str,8,200);
                 l_pic := tpic(m_pic_list.Items[p]).m_pic;
                end;
                if l_pic.Width > paintbox4.Width*0.8 then
                begin
                paintbox4.Canvas.StretchDraw(rect(
                l_margin + 100,
                l_y + 30+ round(paintbox4.Canvas.Textheight(' ')*1.5),
                l_margin+100+round(paintbox4.Width*0.6),
                l_y + 30 + round( l_pic.Height *(paintbox4.Width *0.6)/l_pic.Width)),
                l_pic.Graphic);
                end
                else
                 paintbox4.Canvas.Draw(l_margin + 100,l_y + 30+ round(paintbox4.Canvas.Textheight(' ')*1.5)
                 ,l_pic.Graphic);
                except
                ;
                end;
...
//   paintbox4.Canvas.Font.Size := 22- strtoint(copy(tarea(listbox2.Items.Objects[m]).m_font,2,1))*2;
   ...
   begin
      ...
      l_pic.LoadFromFile(copy(l_str,8,200));
      //paintbox4.Canvas.Draw(l_margin,l_y,l_pic.Graphic);
//      l_y := l_y + l_pic.Graphic.Height +round(paintbox4.Canvas.Textheight(' ')*1.5); ;

       if l_pic.Width > paintbox4.Width*0.8 then
        l_y := l_y + 20 + round( l_pic.Height *(paintbox4.Width *0.6)/l_pic.Width)  +round(paintbox4.Canvas.Textheight(' ')*1.5)
       else
         l_y := l_y + 20+ round(paintbox4.Canvas.Textheight(' ')*1.5)+ l_pic.Height;
      if l_y > paintbox4.Height then
        paintbox4.Height := l_y + 30;
       l_rectbuttom := l_y;
      l_pic.Free;
...
begin
       ...
       exit;
        ...
        begin
         l_tag := copy(listbox2.Items.Strings[memo6.tag],0,1);
         if l_tag  = '[' then
         begin
           l_tag := copy(listbox2.Items.Strings[memo6.tag],5,1);
           if l_tag = ']' then
        begin
