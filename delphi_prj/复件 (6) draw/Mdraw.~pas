...
//2007-05-30 add serial IR control
}
interface
...
uses
  ...
  XMLDoc, OleCtnrs,ComObj, Clipbrd ,Umysql,Math,ShellAPI, Spin, ImgList,
  ValEdit;
...
//    procedure Button13Click(Sender: TObject);
  ...
  public
    ...
    procedure delayns(tick:longword;frq:TLargeInteger);
  end;
...
procedure TForm1.PaintBox2DblClick(Sender: TObject);
begin
    paintbox1.Tag := 0;
end;
...
procedure TForm1.Btn_filterClick(Sender: TObject);
begin
    doc_filter;
end;


procedure TForm1.doc_filter;
var
    i,b,m : integer;
    l_f : pchar;
...
procedure TForm1.Btn_libClick(Sender: TObject);
var
...
procedure TForm1.RB_editClick(Sender: TObject);
begin
...
procedure TForm1.TV_BooksChange(Sender: TObject; Node: TTreeNode);
var 
    sql : string;
    l_item : TListItem;
begin
...
procedure TForm1.TV_BooksEdited(Sender: TObject; Node: TTreeNode;
  var S: String);
var
    i,b : integer;
    l_f : pchar;
...
procedure TForm1.TV_BooksEditing(Sender: TObject; Node: TTreeNode;
  var AllowEdit: Boolean);
begin
     allowedit := false;
     if node.HasChildren then
...
procedure TForm1.Lv_objDblClick(Sender: TObject);
begin
...
procedure TForm1.Memo6Exit(Sender: TObject);
var
    l_f : pchar;
    sql : string;
begin
...
procedure TForm1.Button_recalClick(Sender: TObject);
begin
        doc_recal
end;

procedure TForm1.Memo6KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
    i :integer;
    l_f : pchar;
...
procedure TForm1.rip1Click(Sender: TObject);
var
    i,b : integer;
    l_f : pchar;
    sql : string;
...
procedure TForm1.Poject1Click(Sender: TObject);
var
    b : integer;
...
procedure TForm1.CB_LineClick(Sender: TObject);
begin
     doc_Repaint;
end;

procedure TForm1.PaintBox4DblClick(Sender: TObject);
var
    l_tag : string;
    sql : string;
begin
...
procedure TForm1.TV_BooksDragDrop(Sender, Source: TObject; X, Y: Integer);
var
  AnItem: TTreeNode;
...
procedure TForm1.TV_BooksDragOver(Sender, Source: TObject; X, Y: Integer;
  State: TDragState; var Accept: Boolean);
var
  AnItem: TTreeNode;
begin
...
procedure TForm1.TabSheet15Resize(Sender: TObject);
begin
...
procedure TForm1.TabSheet19Show(Sender: TObject);
begin
   Main.Show;
   Main.Parent := tabsheet19;
end;
...
procedure TForm1.Button_LocateClick(Sender: TObject);
var
  l_bookid : string;
  l_node : TTreenode;
begin
...
procedure TForm1.delayns(tick:longword;frq:TLargeInteger);
var //clk,old_clk: longword;
    clk,old_clk: TLargeInteger;
begin
{   old_clk := GetTickCount;
   clk := old_clk + tick;
   while GetTickCount < clk do
}   QueryPerformanceCounter(old_clk);
//   clk := old_clk + round(tick * frq /1000000);
   clk := old_clk + round(tick * 3.58);
   while  QueryPerformanceCounter(old_clk) do
   begin
      if clk < old_clk then break;
   end;
 //  application.ProcessMessages;
...
var clk,old_clk,diff,dwerror,x,y,z: longword;
  cs: tcomstat;
  a : integer;
  F:   TComStateFlag;
  l_flag : integer;
  ...
  ret : longbool;
  n,m : integer;
  ir:array [1..4] of byte;
  map:array [0..15] of char;
   dd,ee,frq:TLargeInteger;
   dxy :dword;
  l_cur: TPoint;
begin
//  dxy := -1;
  ...
  if ( setcommmask(hcomm,EV_DSR) = false) then
     application.MessageBox('aa','aa');

   ret := false;
  if QueryPerformanceCounter(dd) then
  begin
     QueryPerformanceCounter(ee);
     memo2.Lines.Add(inttostr(ee - dd)+' frq = '+ inttostr(frq));
  end;
  ...
  while (true) do
   begin
      ...
      application.ProcessMessages;
//      sleep(1000);
      ret := waitcommevent(hcomm, dwerror,nil);
      if ret = false then continue;
      //判断起始低电平
      for n := 1 to 10 do
        begin
           delayns(822,frq);
           GetCommModemStatus(hcomm,dwerror);
           if ((dwerror and MS_DSR_ON) = MS_DSR_ON)then
           begin
               l_flag := 0;
               break;
           end;
 	...
 	begin
        ...
        // 等待4740ns delayns(4740,frq);
{        if cb_rep.Checked then
        begin
          delayns(2500,frq);
          //repeat the last press
          GetCommModemStatus(hcomm,dwerror);
          if ((dwerror and MS_DSR_ON) <> MS_DSR_ON)then
          begin
             memo2.Lines.Add(
               '  ' + map[ir[1] div 16] + map[ir[1] mod 16]
             + '--' + map[ir[2] div 16] + map[ir[2] mod 16]
             + '--' + map[ir[3] div 16] + map[ir[3] mod 16]
             + '--' + map[ir[4] div 16] + map[ir[4] mod 16]);
              continue;
          end;
          delayns(2240,frq);
        end
 	...
 	begin
 		...
 		delayns(822,frq); //822ns
 		GetCommModemStatus(hcomm,dwerror);

               if ((dwerror and MS_DSR_ON) = MS_DSR_ON)then
 		begin
 		    ir[n] := ir[n] shl 1 + 1;
 		    delayns(1000,frq);//1000
 		end
 		else
 		begin
                    ir[n] := ir[n] shl 1 ;
        ...
        begin
           ...
           + '--' + map[ir[4] div 16] + map[ir[4] mod 16]);
           //if ir[3]= then
           //     SendMessage(hd,WM_USER,ir[1],ir[3]);
           n := 0;
           if ir[3] = 11*16 +8 then
           mouse_event( MOUSEEVENTF_MOVE,0,10,0,0);
//           dxy := -10;
           if ir[3] = 10*16 +8 then
           mouse_event( MOUSEEVENTF_MOVE,0,n-10,0,0);

           if ir[3] = 5*16 +8 then
           mouse_event( MOUSEEVENTF_MOVE,10,0,0,0);

           if ir[3] = 1*16 +8 then
           mouse_event( MOUSEEVENTF_MOVE,n-10,0,0,0);
        ...
        {
           memo2.Lines.Add(inttostr(gettickcount) + 'error decode :'+
           map[ir[3] div 16] + map[ir[3] mod 16]
           +'--'+ map[ir[4] div 16] + map[ir[4] mod 16]);
         }
...

 {
  if ret then
   begin
     if (dwerror =  EV_RLSD)  then
     begin
       old_clk := clk;
       clk := GetTickCount;
       diff := clk -old_clk;
       if GetCommModemStatus(hcomm,dwerror) then
         if ((dwerror and MS_RLSD_ON) = MS_RLSD_ON)then
         begin
           memo2.Lines.Add(pchar(inttostr(dwerror)+'--'+inttostr(diff)));
         end
         else
         begin
           memo2.Lines.Add(pchar(inttostr(dwerror)+'--'+inttostr(diff)))
...
//  GetCommModemStatus(hcomm,dwerror);
//   application.MessageBox(pchar(inttostr(dwerror)+inttostr(a)),'com');
{
   clearcommerror(hcomm, dwerror, @cs);
//   fCtlHold, fDsrHold, fRlsHold, fXoffHold, fXOffSent, fEof,    fTxim
   f := fCtlHold;
   if (fCtlHold in cs.Flags) then
   begin
      a := a+1;
   end;
   f := fDsrHold;
   if (f in cs.Flags) then
   begin
      a := a+2;
   end;
   f := fRlsHold;
   ...
   begin
      ...
      if (fXoffHold in cs.Flags) then
   begin
      a := a+8;
   end  ;
      if (fXOffSent in cs.Flags) then
   begin
      a := a+16;
   end   ;
    application.MessageBox(pchar(inttostr(dwerror)+inttostr(a)),'com');
...
begin
    g_stop := 1;
    keybd_event($e0,$3f,KEYEVENTF_EXTENDEDKEY,0);
//    keybd_event(VK_RWIN,0,0,0);
//    keybd_event(VK_RWIN,0,KEYEVENTF_KEYUP,0);
    keybd_event($B5,0,0,0);
    keybd_event($B5,0,KEYEVENTF_KEYUP,0);
end;
