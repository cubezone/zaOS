...
type
  ...
  procedure SavetoFile(i:integer; node:ixmlnode);virtual;
  procedure LoadfromFile(i:integer;node:iXMLnode);virtual;
...
procedure Tobj.SavetoFile(i:integer; node:ixmlnode);
var
    vnode,vsubnode : ixmlnode;
    li :integer;
begin
   vnode := node.AddChild('obj');
   ...
   vsubnode := vnode.AddChild('m_image');
   vsubnode.Text := m_image;
   vsubnode := vnode.AddChild('m_code');
   vsubnode.Text := m_code;
   vsubnode := vnode.AddChild('m_seq');
   vsubnode.Text := inttostr(i);
   vsubnode := vnode.AddChild('m_textalign');
   vsubnode.Text := m_textalign;
...
procedure Tobj.LoadFromFile(i:integer;node:iXMLnode);
...
begin
   ...
   m_image:= vnode.ChildNodes['m_image'].text;
   m_code:= vnode.ChildNodes['m_code'].text;
   m_seq:= vnode.ChildNodes['m_seq'].text;
   m_textalign:= vnode.ChildNodes['m_textalign'].text;
   if  classname = 'TTable' then
   begin
...
//?     m_drect.Right := round((r.Left + r.Right)/2)
        ...
        m_drect.Left := round((r.Left + r.Right)/2)
     else
    ...
    begin
     m_drect.Left := r.Left;
     if (abs(m_x-m_sx) > 0) then
     m_drect.Top :=  round(-(m_sx-r.Left)*(m_y-m_sy) /(m_x-m_sx)) + m_sy
     else
     m_drect.Top :=  round((r.Top + r.Bottom)/2)
     {
     begin
    ...
    begin
     ...
     begin
     m_drect.Left := round((r.Right + r.Left)/2) + 1;
     if (m_y > m_sy) then
       m_drect.top := r.Bottom
      else
      begin
...
//   l_ico.LoadFromFile('c:\earth.ico');
   ...
   if m_textalign = 'center' then
    m_canvas.TextOut(round((m_sx+m_x)/2)- length(m_desc)*3,round((m_sy + m_y)/2-10 ),m_desc)
    else
...
begin
  try
   ...
   limage.LoadFromFile(m_image);
    ...
    if not limage.Graphic.Empty then
       m_Canvas.StretchDraw(rect(m_sx,m_sy,m_x,m_y),limage.Graphic);
   // m_Canvas.Brush.Color := m_Canvas.Pixels[round((m_sx + m_x)/2),round((m_sy + m_y)/2)];
    //l_hdc := GetWindowDC(0);
    m_Canvas.Brush.Color  := GetPixel(l_hdc,10,10);
    if (m_Canvas.Brush.Color = -1) then
    begin
    if (m_textalign = 'center')then
       m_Canvas.Brush.Color  := $00F7EEE8
    else
