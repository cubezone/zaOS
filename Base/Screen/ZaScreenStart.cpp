...
void ZaScreenStart::init()
{
	...
	m_lab_time->setSize(200,m_height-120,500,100);
	m_lab_time->setfont(1.0f,1.0f,1.0f,1.0f,30);
	AddChild(m_lab_time);

	m_lab_date = new ZaLabel();
	m_lab_date->m_text = m_dt.GetDateTime((char*)"%a,%b %d,%Y");
	m_lab_date->setfont(0.6f,0.6f,0.6f,1.0f,9);
	m_lab_date->setSize(210,m_height-200,500,100);
	AddChild(m_lab_date);
...
void ZaScreenStart:: tweener(int id, float value)
{
	if(id==10)
	{
		...
		m_sld_c->m_y = 80;
		m_lab_time->m_y = m_height-120;
		m_lab_date->m_y = m_height-200;
		m_bg->m_z =1.0f;
	}
