...
void ZaScreenRead::init()
{
	...
	m_sld_a->setSize(60 , m_height - 90, m_width- 120 , 30);
	AddChild(m_sld_a);
	addListener(m_sld_a, ZaEvent::EVENT_SLIDER_CHANGED, &ZaScreenRead::onclick);

	m_foot = new ZAImage();
	m_foot->LoadImg("app/native/res/bbui/backbar.png", m_width, 140);
	m_foot->setSize((int) m_width / 2, 70, m_width, 140);
	AddChild(m_foot);


	m_labui = new ZaLabel();
	m_labui->m_text = "      Back                 Catalog              Font";
	m_labui->setfont(1.0f,1.0f,1.0f,1.0f,5);
	m_labui->setSize(10,20,m_width,40);
	AddChild(m_labui);

	ZAImage *l_img = new ZAImage();
	l_img->LoadImg("app/native/res/bbui/ic_previous.png", ICONSIZE, ICONSIZE);
	m_back = new ZaButton();
	m_back->setImage(l_img);
	m_back->setSize(30,50, ICONSIZE, ICONSIZE);
	AddChild(m_back);
...
//	addListener(m_copy, ZaEvent::EVENT_CLICK, &ZaScreenRead::onclick);
	...
	l_img = new ZAImage();
	l_img->LoadImg("app/native/res/bbui/ic_catalog.png", ICONSIZE, ICONSIZE);
	m_catalog = new ZaButton();
	m_catalog->setImage(l_img);
	m_catalog->setSize(m_width*0.3, 50, ICONSIZE, ICONSIZE);
	AddChild(m_catalog);
	...
	m_dict->setSize(m_width*0.6, 30, ICONSIZE, ICONSIZE);
//	AddChild(m_dict);
//	addListener(m_dict, ZaEvent::EVENT_CLICK, &ZaScreenRead::onclick);

	l_img = new ZAImage();
	l_img->LoadImg("app/native/res/bbui/ic_font.png", ICONSIZE, ICONSIZE);
	m_font_conf = new ZaButton();
	m_font_conf->setImage(l_img);
	m_font_conf->setSize(m_width - ICONSIZE -40 -240, 50, ICONSIZE, ICONSIZE);
	AddChild(m_font_conf);
	addListener(m_font_conf, ZaEvent::EVENT_CLICK, &ZaScreenRead::onclick);

	m_font_bg = new ZAImage();
	m_font_bg->LoadImg("app/native/res/grey2.png", m_width, m_height *0.7f);
	m_font_bg->setSize(m_width / 2, m_height / 2, m_width, m_height *0.7f);
	AddChild(m_font_bg);

	m_lst_lang = new ZaListLang();

	m_lst_lang->setSize(20, m_height*0.7f, m_width - 20 - 80, m_height*0.12f);
	...
	m_lst_font->setSize(20, m_height*0.45f, m_width - 20 , m_height*0.22f);
	...
	m_lst_color->setSize(20, m_height*0.2f, m_width - 20 - 200, m_height*0.22f);
	...
	m_the_me->setSize(m_width - 160, m_height*0.3f, ICONSIZE, ICONSIZE);
	AddChild(m_the_me);
	...
	m_font_i->setSize(m_width - 100,  m_height*0.7f, ICONSIZE, ICONSIZE);
	AddChild(m_font_i);
	...
	m_font_d->setSize(m_width - 100,  m_height*0.5f, ICONSIZE*0.7, ICONSIZE*0.7);
	AddChild(m_font_d);
...
void ZaScreenRead::menu()
{
	...
	{
//		ZaTweener::getInstance()->push(m_copy, 2, -ICONSIZE, 30, 1, 2);
//		ZaTweener::getInstance()->push(m_catalog, 2, -ICONSIZE, 30, 1, 3);
//		ZaTweener::getInstance()->push(m_dict, 2, -ICONSIZE, 30, 1, 4);
//		ZaTweener::getInstance()->push(m_font_conf, 2, -ICONSIZE, 30, 1, 5);
		m_back->m_visiable = true;
		m_font_conf->m_visiable = true;
		m_catalog->m_visiable = true;
		m_labui->m_visiable = true;
		ZaTweener::getInstance()->push(this, 12, 0, 0, 1, 1);
	}
	else
	{
		m_head->m_visiable = false;
		m_sld_a->m_visiable = false;
		m_back->m_visiable = false;
...
//		ZaTweener::getInstance()->push(m_copy, 2, 30, -ICONSIZE, 1, 2);
//		ZaTweener::getInstance()->push(m_catalog, 2, 30, -ICONSIZE, 1, 3);
//		ZaTweener::getInstance()->push(m_dict, 2, 30, -ICONSIZE, 1, 4);
//		ZaTweener::getInstance()->push(m_font_conf, 2, 30, -ICONSIZE, 1, 5);
//		ZaTweener::getInstance()->push(this, 14, 0, 0, 6, 1);

		m_back->m_visiable = false;
		m_font_conf->m_visiable = false;
		m_catalog->m_visiable = false;
		m_labui->m_visiable = false;
		ZaTweener::getInstance()->push(this, 14, 0, 0, 1, 1);
