...
void ZaTextArea::getSelectText(string & txt)
{
	if (m_select_a.m_visiable)
	{
		...
		if (ret < 0)
		{
			...
			throw CException(__FILE__,__LINE__,(char*)"zaFont::load_font convert error ,filename %s","NULL");
		}
		...
		if (m_height < m_contentheight)
				{
					...
					if (m_top < 0)
						m_top = 0;
					else if (m_top > m_contentheight-m_height +90)
						m_top =  m_contentheight-m_height +90;
				}
		o_x = env->m_x;
		o_y = env->m_y;
	}
