...
void initExternalDisplay()
{
	...
	int miDispNb;
	screen_get_context_property_iv(screen_ctx, SCREEN_PROPERTY_DISPLAY_COUNT,&miDispNb);
	...
	if (miDispNb > 1)
	{
		...
		screen_window_t mScreenWindow2;
		...
		{
			int bar[] = {
				SCREEN_BLIT_COLOR, 0xff000000+a*200,
				SCREEN_BLIT_DESTINATION_X, 90 * a,
				SCREEN_BLIT_DESTINATION_Y, 10* a,
				SCREEN_BLIT_DESTINATION_HEIGHT, 500,
				SCREEN_BLIT_DESTINATION_WIDTH, 20,
				SCREEN_BLIT_END };

			screen_fill(mScreenCtx2, screen_buf[0], bar);
		}
...
int bbutil_init_egl(screen_context_t ctx, enum RENDERING_API api)
{
	...
	EGLint attrib_list[] =
	{ EGL_RED_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_BLUE_SIZE, 8,
	...
	if (screen_create_window_group(screen_win, "com.bbcubezone.bookreader")
			!= 0)
	{
...

	{
		...
		for (ic = 0; ic < iCharMapCnt; ic++)
		{
			...
			fprintf(stderr,
					...
					ic, platform_id, encoding_id, lang);
		}
	}
...
void bbutil_measure_text(font_t* font, const char* msg, float* width,
		float* height)
{
	int i, c;
...
int bbutil_load_texture(const char* filename, int* width, int* height,
...
int bbutil_calculate_dpi(screen_context_t ctx)
{
	...
	rc = screen_get_display_property_iv(screen_disp,
	...
	else
	{
		...
		rc = screen_get_display_property_iv(screen_disp, SCREEN_PROPERTY_SIZE,
				screen_resolution);
		if (rc)
		{
			perror("screen_get_display_property_iv");
		...
		int diagontal_inches = 0.0393700787
				* sqrt(
...
int bbutil_rotate_screen_surface(int angle)
{
	...
	rc = screen_get_window_property_iv(screen_win, SCREEN_PROPERTY_ROTATION,
			&rotation);
	if (rc)
	{
		perror("screen_set_window_property_iv");
	...
	rc = screen_get_window_property_iv(screen_win, SCREEN_PROPERTY_BUFFER_SIZE,
			size);
	if (rc)
	{
		perror("screen_set_window_property_iv");
	...
	if (!skip)
	{
		...
		rc = screen_set_window_property_iv(screen_win,
				SCREEN_PROPERTY_SOURCE_SIZE, size);
		if (rc)
		{
			perror("screen_set_window_property_iv");
			return EXIT_FAILURE;
