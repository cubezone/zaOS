...
void zaFont::initbyPath(const char * path, int size)
{
	...
	{
		if (access("/usr/fonts/font_repository/adobe/AdobeHeitiStd-Regular.otf",
				F_OK) >= 0)
		{
			sprintf(m_path, "%s",
					"/usr/fonts/font_repository/adobe/AdobeHeitiStd-Regular.otf");
		}
		else if (access(
				"/usr/fonts/font_repository/monotype/MHeiB.GB18030_200910.ttf",
				F_OK) >= 0)
		{
			sprintf(m_path, "%s",
					"/usr/fonts/font_repository/monotype/MHeiB.GB18030_200910.ttf");
...
void zaFont::getcharsize(int charcode, int* x, int *y)
{
	...
	if (!l_f)
		throw CException(__FILE__, __LINE__, "getcharsize err ,charcode %s",
				charcode);
	if (l_f)
	{
		*x = l_f->advance[0];
...
void zaFont::setcolor(uint color)
{
	float r, g, b;
	r = (float) ((color & 0xFF0000) >> 16);
	g = (float) ((color & 0x00FF00) >> 8);
	b = (float) ((color & 0x0000FF));
	setcolor(r / 255, g / 255, b / 255, 1.0f);
}
...
int zaFont::load_font(char * instr, int len)
{
	...
	for (int cci = 0; cci < len; cci += 2)
	{
		...
		if (this->m_map.find(charcode) == m_map.end())
		{
			...
			{
				//
				if (FT_Load_Char(face, 0x4E2D + charcode - 0x4E2D,
						FT_LOAD_RENDER))
				{
...
float zaFont::render_text_one(font_t * font, int charcode, float x, float y)
{
	...
	if (!font)
	{
		throw CException(__FILE__, __LINE__, "font is NULL  ,charcode %d",
				charcode);
	}
	float pen_x = 0.0f;
...
// char * formart is UTF-16LE, len is the size of the buf; linesize = 30;
int zaFont::render_text(const char * buf, int len, float x, float y,
		const char * decode, int width, int linesize)
{
...
//			throw CException(__FILE__,__LINE__,"render font error ,no charcode %d",charcode);
			...
			else
			{
				...
				throw CException(__FILE__, __LINE__,
						"load font error ,charcode %d", charcode);
			}
		}
		...
		{
			l_x = x;
			l_y += this->m_max_y + linesize;
			y -= this->m_max_y + linesize;
		}
		p += 2;
	}
	free(in);
...
void zaFont::render_text(const char * buf, int len, float x, float y,
		const char * decode)
{
	iconv_t tt;
	tt = iconv_open("UTF-16LE", decode);
	...
	{
		iconv_close(tt);
		throw CException(__FILE__, __LINE__, "font convert err ,errcode %d",
				errno);
			}
//	if ( ret < 0)
//		fprintf(stderr,"iconv errno %d\n",err);
