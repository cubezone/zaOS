...
ZaRichText::ZaRichText()
{
	...
	m_color_bg = 0x000000;
	m_color_content = 0xffffff;
	m_color_head = 0xffff60;
	m_color_dialog = 0x8888FF;
	m_color_link = 0x00ffa0;
...
void ZaRichText::drawbackground()
{
	if (m_Imgbg)
		m_bg.DrawImg();
	else
		ZAImage::FillRect(this->m_color_bg,0,0,m_width,m_height,0);
}
...
void ZaRichText::drawControl()
{
	if (!m_opened)
	{
		...
		m_font->setcolor(m_color_content);
...
//				int bx = l_s_e->m_x;
			...
			else if ((*iter)->action < 10)
			{
				...
				if ((*iter)->action == 1)
				{
					m_font->setcolor(this->m_color_dialog);
				}
				if ((*iter)->action == 8)
				{
					m_font->setcolor(this->m_color_link);
				}
...
void ZaRichText::openfile(const char *p)
{
	try
	{
		...
		if (zaMySQL::getInstance()->getreadhis(m_filename, &page, &f))
		{
			...
			ZaResLib::getInstance()->m_font_rt->initbyCharSet(m_txt.m_charset,
					0);
			m_txt.m_filepos = f;
			if (m_txt.m_epub.m_pagecur > m_txt.m_epub.m_xml.m_pages.size())
				throw CException(__FILE__, __LINE__,
						"read page is over the real size of book, %d", page);
...
//				drawbackground();
		...
		{
			m_b.m_x += env->m_x - o_x;
			if (m_b.m_x > m_width*1.5)
				m_b.m_x = m_width*1.5;
			if (m_b.m_x < -m_width*0.5)
				m_b.m_x = -m_width*0.5;
			m_a.m_alpha = (float) abs(m_b.m_x - m_width / 2)
					/ (m_width * 3 / 2);
			m_a.m_z = (float) abs(m_b.m_x - m_width / 2)
								/ (m_width);
		}
	...
	if (env->m_type == ZaEvent::EVENT_UP)
	{
		if (m_changing > 0)
		{
			ZaTweener::getInstance()->push(&m_b,ZaTweener::X, m_b.m_x, m_width / 2, 0,
					12);
			ZaTweener::getInstance()->push(&m_a,ZaTweener::ZOOM, m_a.m_z, 0, 0, 12);
			//set change to NO, and save the screen to a.
			ZaTweener::getInstance()->push(this,ZaTweener::LOADSCREEN, 0, 0, 12, 1);
		}
		//post click;
		...
		if (inPoint(env->m_x, env->m_y))
		{
			...
			if(abs(env->m_x-s_x) + abs(env->m_y-s_y)<10)//5
			{
				int x = env->m_x;
				int y = env->m_y;
...
void ZaRichText::getpos(float & percent)
{
	...
	{
		percent = (float) (m_txt.m_epub.m_pagecur
				+ (m_txt.m_filepos) / (float) m_txt.m_size)
				/ (float) m_txt.m_epub.m_xml.m_pages.size();
	}
	else
	{
		percent = (float) (m_txt.m_filepos) / (float) m_txt.m_size;
...
//					}
	...
	fprintf(stderr,"open file  m_filepos %d- m_pagesize %d "
			"m_epub.m_pagecur %d- m_pages.size() %d m_size %d\n",
	m_txt.m_filepos,
	m_txt.m_pagesize,
	m_txt.m_epub.m_pagecur,
