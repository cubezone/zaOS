...
char * CDateTime::GetDateTime(char *format)
{
	...
	if (!strcmp(format,"HMS"))
		...
		strftime(m_temp,20,"%Y%m%d%H%M%S",&m_datetime);
	else if (!strcmp(format,"Ym"))
		strftime(m_temp,20,"%Y%m",&m_datetime);
	else
		strftime(m_temp,20,format,&m_datetime);
	return m_temp;
}

char * CDateTime::GetRawDateTime()
{
	return GetDateTime(( char*)"YmdHMS");
}
...
bool CDateTime::IsDateEffect(const CDateTime &EffDate, const CDateTime &ExpDate)
{
...
char* CDateTime::GetCurTime()
{
	time(&m_time);
	m_localtime = localtime(&m_time);
	m_datetime = *m_localtime;
	memset(m_temp,0,20);
	strftime(m_temp,20,"%Y%m%d%H%M%S",m_localtime);
	return m_temp;
}
