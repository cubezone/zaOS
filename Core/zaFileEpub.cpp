...
//	else
	...
	if (access(l_s, R_OK) < 0 || l_my->getreadhis(m_hash) == 0)
	{
		...
		char *l_sql;
		l_sql = sqlite3_mprintf("delete from tab_catelog  where bookname=%Q;",
				m_hash);
		string ls_sql = l_sql;
		sqlite3_free(l_sql);
		for (unsigned int i = 0; i < m_xml.m_pages.size(); i++)
		{
			l_sql = sqlite3_mprintf(
					"insert into tab_catelog select %Q,'%d',%Q,'%d %q'; ",
					m_hash, i + 1, m_xml.m_pages[i].href.c_str(), i + 1,
	...
	else
	{
		...
		{
			for (int i = l_my->m_column; i < (l_my->m_row + 1) * l_my->m_column;
					i += l_my->m_column)
			{
...
void zaFileEpub::gethtmlpath()
{
	...
	if (m_pagecur < m_xml.m_pages.size())
		sprintf(m_pathname, "%s/%s", m_xml.m_path,
				m_xml.m_pages[m_pagecur].href.c_str());
	else
		throw CException(__FILE__, __LINE__,
				"m_pagecur %d over the book size %d ", m_pagecur,
				m_xml.m_pages.size());
}
