...
void ZaScreenRead::init() {
	...
	m_rt->m_bg.setXY((int) m_width / 2, (int) m_height / 2);
	AddChild(m_rt);
	...
	m_sld_a->setSize(m_width-460,m_height-75,400,30);
	AddChild(m_sld_a);
	...
	m_dict->setSize(m_width-210, 30, 48, 48);
	AddChild(m_dict);
	addListener(m_dict, ZaEvent::EVENT_CLICK, &ZaScreenRead::onclick);

	l_img = new ZAImage();
	l_img->LoadImg("app/native/res/font_i.png", 48, 48);
	m_font_conf = new ZaButton();
	m_font_conf->setImage(l_img);
	m_font_conf->setSize(m_width-70, 30, 48, 48);
	AddChild(m_font_conf);
	addListener(m_font_conf, ZaEvent::EVENT_CLICK, &ZaScreenRead::onclick);

	m_font_bg = new ZAImage();
	m_font_bg->LoadImg("app/native/res/grey2.png",m_width,m_height/2+100);
	m_font_bg->setSize(m_width/2,m_height/2,m_width,m_height/2+100);
	AddChild(m_font_bg);

	m_lst_lang= new ZaListLang();
	m_lst_lang->setSize(20,350,200,m_height-600);
	...
	l_img = new ZAImage();
	l_img->LoadImg("app/native/res/font_i.png", 32, 32);
	m_font_d = new ZaButton();
	m_font_d->setImage(l_img);
	m_font_d->setSize(m_width/2-250,270, 32, 32);
	AddChild(m_font_d);
	addListener(m_font_d, ZaEvent::EVENT_CLICK, &ZaScreenRead::onclick);

	m_lst_font= new ZaListFont();
	m_lst_font->setSize(m_width/2-20,350,m_width/2,m_height-600);
	...
	//m_font_i->setSize(m_width-70, 30, 48, 48);
	m_font_i->setSize(m_width/2+160, 270, 48, 48);
	AddChild(m_font_i);
	addListener(m_font_i, ZaEvent::EVENT_CLICK, &ZaScreenRead::onclick);

	l_img = new ZAImage();
	l_img->LoadImg("app/native/res/Theme48.png", 48, 48);
	m_the_me = new ZaButton();
	m_the_me->setImage(l_img);
	m_the_me->setSize(m_width/2-20, 270, 48, 48);
	AddChild(m_the_me);
	addListener(m_the_me, ZaEvent::EVENT_CLICK, &ZaScreenRead::onclick);

	m_scroll = new ZaScroller();
	m_scroll->setSize(10,(m_height-(m_height/2+100))/2,m_width-20,m_height/2+100);
	m_lab = new ZaLabel();
	m_lab->m_text = "Desc:";
	m_lab->setSize(0,m_scroll->m_height-50,m_scroll->m_width,m_scroll->m_height-50);
	...
	m_delete->setSize(m_width-60,m_scroll->m_y+m_scroll->m_height-60,48,48);
	...
	std::string dict("/accounts/1000/shared/books/dict/oxford_advanced_learner_dictionary");
...
void ZaScreenRead::menu()
{
	...
	if (m_foot->m_visiable) {
		ZaTweener::getInstance()->push(m_copy, 2, -60, 30, 1, 2);
		ZaTweener::getInstance()->push(m_catalog, 2, -60, 30, 1, 4);
		ZaTweener::getInstance()->push(m_dict, 2, -60, 30, 1, 7);
		ZaTweener::getInstance()->push(m_font_conf, 2, -60, 30, 1, 10);
		ZaTweener::getInstance()->push(this, 12, 0,0, 11, 1);
	} else {
		...
		ZaTweener::getInstance()->push(m_copy, 2, 30,-60, 1, 2);
		ZaTweener::getInstance()->push(m_catalog, 2, 30,-60, 1, 4);
		ZaTweener::getInstance()->push(m_dict, 2, 30,-60, 1, 7);
		ZaTweener::getInstance()->push(m_font_conf, 2, 30,-60, 1, 10);
		ZaTweener::getInstance()->push(this, 14, 0,0, 11, 1);
	}
...
void ZaScreenRead::recHis() {
	...
	char * l_sql;
	l_sql = sqlite3_mprintf( "delete from tab_bookmark where bookURL =  \"%s\" ;",
			m_rt->m_filename);
...
//	if (strcmp(m_rt->m_filename + strlen(m_rt->m_filename)-4,"epub") == 0)
	{
		...
		{
			l_sql = sqlite3_mprintf("insert into tab_bookmark select %Q,%Q,%Q,%Q,"
						"%Q,%Q,'%d','%d',%Q ;",
						m_rt->m_filename,
						dt.GetDateTime(),
						m_rt->m_txt.m_hash,
...
void ZaScreenRead::onclick(ZaObj * obj, ZaEvent *env) {
	...
	if (obj == m_copy)
	{
		...
		else
		{
			...
			ZaTweener::getInstance()->push(&m_rt->m_select_a,ZaTweener::X,0,m_width/2-100,0,7);
			ZaTweener::getInstance()->push(&m_rt->m_select_a,ZaTweener::Y,0,m_height/2+100,0,7);
			ZaTweener::getInstance()->push(&m_rt->m_select_b,ZaTweener::X,0,m_width/2+100,0,7);
			ZaTweener::getInstance()->push(&m_rt->m_select_b,ZaTweener::Y,0,m_height/2-100,0,7);
		}
...
//			m_scroll->m_top = 0;
			...
			if(l_s.length()>0)
			{
				...
				ZaScreenBase * l_src =
						(ZaScreenBase *) ZaResLib::getInstance()->getScreen("ZaScreenDict");
		...
		else
		{
			m_rt->m_select_a.m_visiable = true;
			m_rt->m_select_b.m_visiable = true;
			ZaTweener::getInstance()->push(&m_rt->m_select_a,ZaTweener::X,m_dict->m_x,m_width/2-100,0,7);
			ZaTweener::getInstance()->push(&m_rt->m_select_a,ZaTweener::Y,0,m_height/2+100,0,7);
			ZaTweener::getInstance()->push(&m_rt->m_select_b,ZaTweener::X,m_dict->m_x,m_width/2+100,0,7);
			ZaTweener::getInstance()->push(&m_rt->m_select_b,ZaTweener::Y,0,m_height/2-100,0,7);
		}
	}
	else if (obj == m_input)
	...
	{
		m_rt->m_font->initbyPath(m_lst_font->m_path[m_lst_font->m_index].c_str(),m_rt->m_font->m_size);
	...
	else if(obj == m_catalog)
	{
		...
		ZaScreenBase * l_src = (ZaScreenBase *)ZaResLib::getInstance()->getScreen("ZaScreenCatalog");
		...
		ZaScreenFlash * l_flash =
				(ZaScreenFlash *) ZaResLib::getInstance()->getScreen("ZaScreenFlash");
		ZaScreenBase * l_src =
				(ZaScreenBase *) ZaResLib::getInstance()->getScreen("ZaScreenHis");
...
void ZaScreenRead::tweener(int id, float value) {
	...
	if (id == 10) {
		...
		{
			int n = atoi(ZaTweener::getInstance()->m_param.c_str());
