...
//	ZaResLib::getInstance()->addScreen("ZaScreenDict",l_main);
	...
	l_main = new ZaScreenStart();
	l_main->setSize(0,0,m_width,m_height);
	l_main->init();
	ZaResLib::getInstance()->addScreen("ZaScreenStart",l_main);

	l_main = new ZaScreenDesktop();
	l_main->setSize(0,0,m_width,m_height);
	l_main->init();
	ZaResLib::getInstance()->addScreen("ZaScreenDesktop",l_main);

	m_active = (ZaScreenBase *)ZaResLib::getInstance()->getScreen("ZaScreenStart");
