...
{
	fprintf(stderr,"ZaSliderSwitch drawControl %p\n",this);
	if (m_img == NULL)
	{
		m_img = ZaResLib::getInstance()->m_jpg;
	}

	m_img->setXY(m_x+m_width/2-m_x+3,m_height/2);
	m_img->m_alpha = 1.0f-m_percent*m_alpha;
	m_img->m_r = m_r;
	m_img->m_z = m_z;
	m_img->drawControl();
...
//    	m_lb->draw();
	...
	if (m_dot)
	{
		m_dot->setXY(m_percent*m_width,m_height/2+6);
		m_dot->drawControl();
	}
}
