...
int ZAImage::DrawImg(int x , int y, int r )
{
	...
	float l_z = sqrt(m_z);
	shadow_vertices[0] = -m_width/2*l_z;
	shadow_vertices[1] = -m_height/2*l_z;
	shadow_vertices[2] = m_width/2*l_z;
	shadow_vertices[3] = -m_height/2*l_z;
	shadow_vertices[4] = -m_width/2*l_z;
	shadow_vertices[5] = m_height/2*l_z;
	shadow_vertices[6] = m_width/2*l_z;
	shadow_vertices[7] = m_height/2*l_z;
...
int ZAImage::load_memory(const char* filename, int* width, int* height, float* tex_x, float* tex_y, GLuint *tex)
{
    GLuint format;
...
/*
    ...
    glRenderbufferStorageOES(GL_RENDERBUFFER_OES, GL_DEPTH_COMPONENT16_OES, GL_RENDERBUFFER_WIDTH_OES, GL_RENDERBUFFER_HEIGHT_OES);
