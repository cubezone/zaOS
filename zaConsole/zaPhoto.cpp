...
int popentest(char * p)
{
	  //"python3.2 app/native/res/helloworld.py"
	    ...
	    if (read_fp != NULL)
	    {
	        ...
	        fprintf(stderr,"Output:/n%s/n", buffer);
	     }
...
int ldir()
{
	  ...
	  sprintf(infile,"/usr/%s",ptr->d_name);
...
int opgltest()
{
	...
	int unit;
	unsigned hwi_off;
	//int ret = hwi_find("pci",unit,hwi_off);
	fprintf(stderr,"fdsafa %d %d",m_gl.m_id,getpid());
	m_gl.init();
	m_gl.initBG();
	m_gl.start();
	return 0;
