 ...
 * 
 */
public final class za_remote extends MainScreen implements
	...
	public za_remote(BluetoothSerialPortInfo in_info) {
		// Fill a 1k array with the a character.
		Arrays.fill(_receiveBuffer, (byte) 'a');
		info = in_info;
		// Initialize the buffers.
		_data = new StringBuffer();
		_data.append("Media Control V2.0.0\n\n朱兴宇\n\ndaemon@wo.com.cn\n\n2010/07/20\n\n\n\n\n>");
		_db = new DataBuffer();
	//  setBackground(BackgroundFactory.createSolidBackground(0x00000000));
		...
		if (info == null) {
			...
			// _port = new BluetoothSerialPort("Hi there",
			...
			// BluetoothSerialPort.FLOW_CONTROL_NONE, 1024, 1024, this);
			_deviceName = "unknown";
		} else {
			// Connect to the selected device.
			_rtf = new RichTextField(_data.toString(),
					RichTextField.USE_ALL_HEIGHT | RichTextField.FOCUSABLE) 
			{
	...
	protected boolean touchEvent(TouchEvent message) {
		...
		case TouchEvent.DOWN:
			// process DOWN here
			m_stat = "down";
			m_finger_x = message.getX(1);
			m_finger_y = message.getY(1);
			l_event = "$A%1"+String.valueOf(m_finger_x)+"%2"+String.valueOf(m_finger_y)+"%3";
			if (_port != null) {
				writeData(l_event.getBytes(),0,l_event.length());
				_data.append(l_event);
				_rtf.setText(_data.toString());
			}
			break;
		case TouchEvent.UP:
		...
		case TouchEvent.MOVE:
			if (m_stat == "down") {
				int t_x = message.getX(1);
				int t_y = message.getY(1);				
				l_event = "$B%1"+String.valueOf(m_finger_x)+"%2"+String.valueOf(m_finger_y)+"%3";
				if (_port != null) {
					writeData(l_event.getBytes(),0,l_event.length());
					_data.append(l_event);
					_rtf.setText(_data.toString());
	...
	/*
	 * private MenuItem _dsrOn = new MenuItem("DSR on", 40, 40) { public void
	 * run() { Status.show( "DSR on" ); try { _port.setDsr(true); }
	 * catch(IOException ioex) { Status.show("Error: " + ioex.toString()); } }
	 * };
	 * 
	 * private MenuItem _dsrOff = new MenuItem("DSR off", 50, 50) { public void
	 * run() { Status.show("DSR off"); try { _port.setDsr(false); }
	 * catch(IOException ioex) { Status.show("Error: " + ioex.toString()); } }
	 * };
	 * 
	 * private MenuItem _enableLoopback = new MenuItem("Enable loopback", 60,
	 * 60) { public void run() { _loopback = true; } };
	 * 
	 * private MenuItem _disableLoopback = new MenuItem("Disable loopback", 70,
	 * 70) { public void run() { _loopback = false; } };
	 * 
	 * private MenuItem _send1k = new MenuItem("Send 1k", 80, 80) { public void
	 * run() { writeData(_receiveBuffer, 0, _receiveBuffer.length); } };
	 */
}
